...
/* ============ ONBOARDING ============ */
const OnboardingScreen=({onComplete})=>{const[step,setStep]=useState(0);const[cls,setCls]=useState(null);const[board,setBoard]=useState(null);const[subjects,setSubjects]=useState([]);const[dqi,setDqi]=useState(0);const[dans,setDans]=useState({});
...
if(step===1)return <div style={{display:"flex",flexDirection:"column",gap:20,padding:20}}><div><p style={{fontSize:13,color:C.textMuted,margin:"0 0 4px"}}>Step 1 of 3</p><h2 style={{fontSize:22,fontWeight:800,color:C.text,margin:0}}>About you</h2></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>{[6,7,8].map(c=><button key={c} onClick={()=>setCls(c)} style={{height:72,borderRadius:16,border:`2px solid ${cls===c?C.primary:C.border}`,background:cls===c?C.primarySoft:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:20,fontWeight:800,color:cls===c?C.primary:C.text}}>Class {c}</button>)}</div><div style={{display:"flex",gap:10}}>{["CBSE","ICSE","State"].map(b=><button key={b} onClick={()=>setBoard(b)} style={{flex:1,height:48,borderRadius:12,border:`2px solid ${board===b?C.accent:C.border}`,background:board===b?C.accentSoft:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:14,fontWeight:600,color:board===b?C.accent:C.text}}>{b}</button>)}</div><div style={{display:"flex",gap:10}}>{["Science","Math"].map(s=>{const sel=subjects.includes(s);return <button key={s} onClick={()=>setSubjects(sel?subjects.filter(x=>x!==s):[...subjects,s])} style={{flex:1,height:48,borderRadius:12,border:`2px solid ${sel?C.success:C.border}`,background:sel?C.successSoft:"transparent",cursor:"pointer",fontFamily:"inherit",fontSize:14,fontWeight:600,color:sel?C.successDark:C.text,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>{sel&&<I n="check" s={16} c={C.success} w={2.5}/>}{s}</button>;})}</div><Btn full onClick={()=>setStep(2)} variant={cls&&board&&subjects.length?"primary":"ghost"}>Continue</Btn></div>;
