...
/* ============ ONBOARDING ============ */
const OnboardingScreen=({onComplete})=>{const[step,setStep]=useState(0);const[dqi,setDqi]=useState(0);const[dans,setDans]=useState({});const[answered,setAnswered]=useState(null);
const diagQs=[{q:"What is 3/4 + 1/2?",opts:["5/4","4/6","1","5/6"],correct:0,topic:"Math"},{q:"Heat travels through vacuum by:",opts:["Conduction","Convection","Radiation","All"],correct:2,topic:"Science"},{q:"Simplify: 2x + 3x",opts:["5x","6x","5x\u00B2","23x"],correct:0,topic:"Math"}];
const features=[
  {icon:"brain",title:"AI Tutor",desc:"Get explanations in 6 different styles",color:"#7C3AED"},
  {icon:"chart",title:"Smart Journey",desc:"Learn \u2192 Teach \u2192 Practice path",color:"#2563EB"},
  {icon:"star",title:"Earn XP",desc:"Level up as you learn more",color:"#F59E0B"},
];
if(step===0)return <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"90vh",gap:0,padding:"24px 20px",textAlign:"center",background:"linear-gradient(180deg, #FAFBFF 0%, #EFF6FF 50%, #F3E8FF 100%)"}}>
  <style>{`@keyframes onbFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}} @keyframes onbFadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} @keyframes onbPulse{0%,100%{transform:scale(1);opacity:0.3}50%{transform:scale(1.15);opacity:0.15}}`}</style>
  <div style={{position:"relative",marginBottom:32}}>
    <div style={{position:"absolute",inset:-12,borderRadius:30,background:"linear-gradient(135deg, rgba(37,99,235,0.12), rgba(124,58,237,0.12))",animation:"onbPulse 3s ease-in-out infinite"}}/>
    <div style={{position:"relative",width:88,height:88,borderRadius:24,background:"linear-gradient(135deg, #2563EB, #7C3AED)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 16px 48px rgba(37,99,235,0.3)",animation:"onbFloat 4s ease-in-out infinite"}}>
      <I n="spark" s={44} c="#fff" w={2}/>
    </div>
  </div>
  <h1 style={{fontSize:32,fontWeight:800,color:C.text,margin:"0 0 8px",letterSpacing:-0.5,animation:"onbFadeUp 0.5s ease 0.1s both"}}>Welcome to <span style={{background:"linear-gradient(135deg, #2563EB, #7C3AED)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>LearnAI</span></h1>
  <p style={{fontSize:16,color:C.textMuted,margin:"0 0 36px",lineHeight:1.6,maxWidth:300,animation:"onbFadeUp 0.5s ease 0.2s both"}}>Your personal AI learning companion that adapts to how you think</p>
  <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:340,marginBottom:36}}>
    {features.map((f,i)=><div key={f.title} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 18px",borderRadius:16,background:"#fff",boxShadow:"0 2px 8px rgba(0,0,0,0.04)",animation:`onbFadeUp 0.5s ease ${0.3+i*0.1}s both`}}>
      <div style={{width:42,height:42,borderRadius:12,background:`${f.color}10`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><I n={f.icon} s={22} c={f.color} w={2}/></div>
      <div style={{textAlign:"left"}}><p style={{fontSize:14,fontWeight:700,color:C.text,margin:0}}>{f.title}</p><p style={{fontSize:12,color:C.textMuted,margin:"2px 0 0"}}>{f.desc}</p></div>
    </div>)}
  </div>
  <div style={{width:"100%",maxWidth:340,animation:"onbFadeUp 0.5s ease 0.6s both"}}><Btn full onClick={()=>setStep(1)} style={{height:52,fontSize:16,borderRadius:16,background:"linear-gradient(135deg, #2563EB, #7C3AED)",boxShadow:"0 8px 24px rgba(37,99,235,0.3)"}}>Get Started</Btn></div>
  <p style={{fontSize:12,color:C.textFaint,margin:"16px 0 0",animation:"onbFadeUp 0.5s ease 0.7s both"}}>Takes less than 2 minutes</p>
</div>;
const done=dqi>=diagQs.length;if(done)return <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"80vh",gap:0,padding:"24px 20px",textAlign:"center"}}>
  <style>{`@keyframes onbScale{from{transform:scale(0.5);opacity:0}to{transform:scale(1);opacity:1}} @keyframes onbFadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}`}</style>
  <Confetti show={true}/>
  <div style={{animation:"onbScale 0.5s cubic-bezier(0.34,1.56,0.64,1) both",marginBottom:24}}>
    <Ring value={100} size={110} stroke={8} color={C.success}><I n="check" s={40} c={C.success} w={2.5}/></Ring>
  </div>
  <h2 style={{fontSize:26,fontWeight:800,color:C.text,margin:"0 0 8px",animation:"onbFadeUp 0.5s ease 0.2s both"}}>You're all set!</h2>
  <p style={{fontSize:15,color:C.textMuted,margin:"0 0 24px",animation:"onbFadeUp 0.5s ease 0.3s both"}}>We've personalized your learning path</p>
  <div style={{display:"flex",gap:24,marginBottom:32,animation:"onbFadeUp 0.5s ease 0.4s both"}}>
    {[{label:"Topics",value:"5",icon:"book"},{label:"XP Goal",value:"80",icon:"star"},{label:"Time",value:"~10m",icon:"clock"}].map(s=><div key={s.label} style={{textAlign:"center"}}>
      <div style={{width:48,height:48,borderRadius:14,background:C.primarySoft,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 6px"}}><I n={s.icon} s={22} c={C.primary}/></div>
      <p style={{fontSize:16,fontWeight:800,color:C.text,margin:0}}>{s.value}</p>
      <p style={{fontSize:11,color:C.textMuted,margin:"2px 0 0"}}>{s.label}</p>
    </div>)}
  </div>
  <div style={{width:"100%",maxWidth:340,animation:"onbFadeUp 0.5s ease 0.5s both"}}><Btn full onClick={onComplete} variant="accent" style={{height:52,fontSize:16,borderRadius:16,boxShadow:"0 8px 24px rgba(37,99,235,0.25)"}}>Start Learning</Btn></div>
</div>;
...
return <div style={{display:"flex",flexDirection:"column",gap:0,padding:"24px 20px",minHeight:"80vh"}}>
  <style>{`@keyframes onbFadeUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} @keyframes optPop{from{opacity:0;transform:scale(0.95)}to{opacity:1;transform:scale(1)}} @keyframes correctFlash{0%{background:${C.primarySoft}}50%{background:#D1FAE5}100%{background:${C.primarySoft}}}`}</style>
  <div style={{marginBottom:20}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
      <div><p style={{fontSize:12,color:C.textMuted,margin:"0 0 2px",fontWeight:500,textTransform:"uppercase",letterSpacing:0.5}}>Quick diagnostic</p><h2 style={{fontSize:22,fontWeight:800,color:C.text,margin:0}}>Question {dqi+1} of {diagQs.length}</h2></div>
      <div style={{padding:"6px 14px",borderRadius:10,background:`${cur.topic==="Math"?C.primary:C.accent}10`}}><span style={{fontSize:12,fontWeight:700,color:cur.topic==="Math"?C.primary:C.accent}}>{cur.topic}</span></div>
    </div>
    <div style={{height:6,borderRadius:3,background:C.borderSoft,overflow:"hidden"}}><div style={{height:"100%",borderRadius:3,background:"linear-gradient(90deg, #2563EB, #7C3AED)",width:`${progress}%`,transition:"width 0.5s cubic-bezier(0.4,0,0.2,1)"}}/></div>
  </div>
  <div style={{background:"#fff",borderRadius:20,padding:"24px 20px",boxShadow:"0 2px 12px rgba(0,0,0,0.04)",border:`1px solid ${C.border}`,marginBottom:20,animation:"onbFadeUp 0.4s ease both"}}>
    <p style={{fontSize:18,fontWeight:700,color:C.text,margin:0,lineHeight:1.5}}>{cur.q}</p>
  </div>
  <div style={{display:"flex",flexDirection:"column",gap:10}}>
    {cur.opts.map((o,i)=>{
      const selected=answered===i;const isCorrect=i===cur.correct;
      const showResult=answered!==null;
      let bg="transparent",borderC=C.border,textC=C.text;
      if(showResult&&selected&&isCorrect){bg="#D1FAE5";borderC="#10B981";textC="#065F46";}
      else if(showResult&&selected&&!isCorrect){bg="#FEE2E2";borderC="#EF4444";textC="#991B1B";}
      else if(showResult&&isCorrect){bg="#D1FAE5";borderC="#10B981";textC="#065F46";}
      else if(selected){bg=C.primarySoft;borderC=C.primary;}
      return <button key={i} onClick={()=>{if(answered!==null)return;setAnswered(i);setTimeout(()=>{setDans({...dans,[dqi]:i});setAnswered(null);setDqi(dqi+1);},800);}} style={{
        padding:"16px 18px",borderRadius:16,border:`2px solid ${borderC}`,background:bg,
        cursor:answered!==null?"default":"pointer",fontFamily:"inherit",fontSize:15,fontWeight:selected?600:500,color:textC,textAlign:"left",
        transition:"all 0.2s ease",display:"flex",alignItems:"center",gap:12,
        animation:`optPop 0.3s ease ${i*0.06}s both`,
      }}>
        <div style={{width:32,height:32,borderRadius:10,background:showResult&&(selected||isCorrect)?borderC+"15":C.borderSoft,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontWeight:700,fontSize:13,color:showResult&&(selected||isCorrect)?borderC:C.textMuted}}>
          {showResult&&isCorrect?"\u2713":showResult&&selected&&!isCorrect?"\u2717":String.fromCharCode(65+i)}
        </div>
        {o}
      </button>;
    })}
  ...
  <div style={{display:"flex",justifyContent:"center",gap:8,marginTop:24}}>
    {diagQs.map((_,i)=><div key={i} style={{width:i===dqi?24:8,height:8,borderRadius:4,background:i<dqi?"linear-gradient(90deg, #2563EB, #7C3AED)":i===dqi?C.primary:C.borderSoft,transition:"all 0.3s ease"}}/>)}
  </div>
</div>;};
